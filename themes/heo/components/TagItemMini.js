...
const TagItemMini = ({ tag, selected = false }) => {
  return (
    <Link
      ...
      <div className='font-light flex items-center'><HashTag className='text-hexo-black-gray dark:text-gray-300 stroke-2 mr-0.5 w-3 h-3'/> {tag.name + (tag.count ? `(${tag.count})` : '')} </div>
    </Link>
  )
}
